...
import { ProductType } from "@/util/dataType";
...
const MenuItem: React.FC<MenuItemProps> = ({ item }) => {
  ...
  const handleAddToCart = () => {
    ...
    dispatch(addToCart({ ...item, quantity: additems, id: item.id || 0 })); // Ensure quantity is added
    setSelected(true);
  };

  const handleRemoveFromCart = () => {
    if (additems === 1) {
      dispatch(removeFromCart({ id: item?.id || 0 }));
      setAddItems(0);
      setSelected(false);
      return;
    }
    setAddItems((prev) => prev - 1);
    dispatch(decrementQuantity({ id: item?.id || 0 }));
  };

  const handleIncrement = () => {
    setAddItems((prev) => prev + 1);
    dispatch(incrementQuantity({ id: item?.id || 0 }));
  };
