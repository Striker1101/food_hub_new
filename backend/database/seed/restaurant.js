...
const seedRestaurant = async (count = 19) => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      restaurants.push({
        ...
        images: dataSet[i].images,
        featuredImage: dataSet[i].featured_image,
        deliveryTime: dataSet[i].time,
      });
    }

    await Restaurant.bulkCreate(restaurants);
    console.log(`${count} restaurants seeded successfully.`);
  } catch (error) {
